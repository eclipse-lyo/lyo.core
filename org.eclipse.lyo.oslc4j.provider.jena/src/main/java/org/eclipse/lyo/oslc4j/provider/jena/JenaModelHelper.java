...
public final class JenaModelHelper {
    ...
    public static Object fromJenaResource(final Resource resource, final Class<?> beanClass)
        ...
        final Map<Class<?>, Map<String, Method>> classPropertyDefinitionsToSetMethods = new
                HashMap<>();
        final Map<String, Object> visitedResources = new HashMap<>();
        final HashSet<String> rdfTypes = new HashSet<>();

        fromResource(classPropertyDefinitionsToSetMethods, beanClass, newInstance, resource,
                visitedResources, rdfTypes);
    ...
    static Model createJenaModel(final String descriptionAbout, final String responseInfoAbout,
            final ResponseInfo<?> responseInfo, final Object[] objects,
        ...
        if (descriptionAbout != null) {
            ...
            handleExtendedProperties(FilteredResource.class, model, descriptionResource,
                    responseInfo.getContainer(), properties, visitedResources);
            ...
            if (responseInfoAbout != null) {
                ...
                final int totalCount = responseInfo.totalCount() == null ? objects.length :
                        responseInfo
                        .totalCount();
                responseInfoResource.addProperty(
                        model.createProperty(OslcConstants.OSLC_CORE_NAMESPACE,
                                PROPERTY_TOTAL_COUNT), model.createTypedLiteral(totalCount));
                ...
                handleExtendedProperties(ResponseInfo.class, model, responseInfoResource,
                        responseInfo, properties, visitedResources);
            }
        }

        // add global namespace mappings
        namespaceMappings.putAll(
                OslcGlobalNamespaceProvider.getInstance().getPrefixDefinitionMap());
        ...
        if (descriptionAbout != null) {
            ...
            ensureNamespacePrefix(OslcConstants.RDF_NAMESPACE_PREFIX, OslcConstants.RDF_NAMESPACE,
                    namespaceMappings);

            // Ensure we have an rdfs prefix
            ensureNamespacePrefix(OslcConstants.RDFS_NAMESPACE_PREFIX, OslcConstants.RDFS_NAMESPACE,
                    namespaceMappings);

            if (responseInfoAbout != null) {
                // Ensure we have an oslc prefix
                ensureNamespacePrefix(OslcConstants.OSLC_CORE_NAMESPACE_PREFIX,
                        OslcConstants.OSLC_CORE_NAMESPACE, namespaceMappings);
            }
        }
    ...
    private static void handleSingleResource(final Resource descriptionResource,
            final Object object, final Model model, final Map<String, String> namespaceMappings,
            final Map<String, Object> properties)
            throws DatatypeConfigurationException, IllegalAccessException, IllegalArgumentException,
            InvocationTargetException, OslcCoreApplicationException {
    ...
    private static List<Object> createObjectResultList(final Class<?> beanClass,
            ...
            throws IllegalAccessException, InstantiationException, DatatypeConfigurationException,
    ...
    private static void fromResource(
            ...
            final Class<?> beanClass, final Object bean, final Resource resource,
            Map<String, Object> visitedResources, HashSet<String> rdfTypes)
            throws DatatypeConfigurationException, IllegalAccessException, IllegalArgumentException,
            InstantiationException, InvocationTargetException, OslcCoreApplicationException,
            URISyntaxException, SecurityException, NoSuchMethodException {
            ...
            if (setMethod == null) {
                if (RDF_TYPE_URI.equals(uri)) {
                    ...
                    if (extendedProperties == null) {
                        logger.fine(
                                "Set method not found for object type:	" + beanClass.getName() +
                                        ", uri:	" + uri);
                    } else {

                        String prefix = resource.getModel().getNsURIPrefix(
                ...
                if (multiple) {
                    setMethodComponentParameterClass = setMethodComponentParameterClass
                            .getComponentType();
                ...
                if (multiple && object.isResource() && ((object.asResource().hasProperty(
                        RDF.first) && object.asResource().hasProperty(RDF.rest)) || (RDF.nil.equals(
                        object)) || object.canAs(RDFList.class))) {
                    ...
                    ExtendedIterator<RDFNode> iterator = object.asResource().listProperties(
                            RDFS.member).
                ...
                if (IReifiedResource.class.isAssignableFrom(setMethodComponentParameterClass)) {
                    ...
                    final Type genericType = setMethodComponentParameterClass
                ...
                for (RDFNode o : objects) {
                    ...
                    if (o.isLiteral()) {
                        ...
                        if (String.class == setMethodComponentParameterClass) {
                            ...
                            // TRUE.
                            if ((Boolean.TRUE.toString().equals(stringValue)) || ("1".equals(
                                    stringValue))) {
                                parameter = Boolean.TRUE;
                            }
                            // XML supports both 'false' and '0' for a false Boolean.
                            else if ((Boolean.FALSE.toString().equals(stringValue)) || ("0".equals(
                                    stringValue))) {
                        ...
                        } else if ((Byte.class == setMethodComponentParameterClass) || (Byte.TYPE
                                == setMethodComponentParameterClass)) {
                            parameter = Byte.valueOf(stringValue);
                        } else if ((Short.class == setMethodComponentParameterClass) || (Short
                                .TYPE == setMethodComponentParameterClass)) {
                        ...
                        } else if ((Long.class == setMethodComponentParameterClass) || (Long.TYPE
                                == setMethodComponentParameterClass)) {
                        ...
                        } else if ((Float.class == setMethodComponentParameterClass) || (Float
                                .TYPE == setMethodComponentParameterClass)) {
                            parameter = XSDDatatype.XSDfloat.parseValidated(stringValue);
                        } else if ((Double.class == setMethodComponentParameterClass) || (Double
                                .TYPE == setMethodComponentParameterClass)) {
                            parameter = XSDDatatype.XSDdouble.parseValidated(stringValue);
                        } else if (Date.class == setMethodComponentParameterClass) {
                            parameter = DatatypeFactory.newInstance().newXMLGregorianCalendar(
                                    stringValue).toGregorianCalendar().getTime();
                        }
                        ...
                        if (URI.class == setMethodComponentParameterClass) {
                            ...
                            if (nestedResourceURIString != null) {
                                ...
                                if (OSLC4JUtils.relativeURIsAreDisabled() && !nestedResourceURI
                                        .isAbsolute()) {
                                    throw new OslcCoreRelativeURIException(beanClass,
                                            setMethod.getName(), nestedResourceURI);
                                }

                                parameter = nestedResourceURI;
                            }
                        } else {
                            final Object nestedBean = setMethodComponentParameterClass
                                    .newInstance();

                            fromResource(classPropertyDefinitionsToSetMethods,
                                    setMethodComponentParameterClass, nestedBean, nestedResource,
                                    visitedResources, rdfTypes);
                    ...
                    if (parameter != null) {
                        if (reifiedClass != null) {
                            ...
                            while (rsIter.hasNext()) {
                                ...
                                fromResource(classPropertyDefinitionsToSetMethods, reifiedClass,
                                        reifiedResource, reifiedStatement, visitedResources,
                                        rdfTypes);
                            }
                        ...
                        if (multiple) {
                            List<Object> values = propertyDefinitionsToArrayValues.computeIfAbsent(
                                    uri, k -> new ArrayList<>());
        ...
        for (final Map.Entry<String, List<Object>> propertyDefinitionToArrayValues :
                propertyDefinitionsToArrayValues
                .entrySet()) {
            ...
            // Else - we are dealing with a collection or a subclass of collection
                ...
                if ((Collection.class == parameterClass) || (List.class == parameterClass) ||
                        (Deque.class == parameterClass) || (Queue.class == parameterClass) ||
                        (AbstractCollection.class == parameterClass) || (AbstractList.class ==
                        parameterClass) || (AbstractSequentialList.class == parameterClass)) {
                ...
                else if ((SortedSet.class == parameterClass) || (NavigableSet.class ==
                        parameterClass)) {
     ...
     */
    private static HashSet<String> getTypesFromResource(final Resource resource,
    ...
    private static boolean isRdfCollectionResource(Model model, RDFNode object) {
        if (object.isResource()) {
            ...
            if (resource.hasProperty(RDF.type,
                    model.getResource(RDF_TYPE_URI + RDF_ALT)) || resource.hasProperty(RDF.type,
                    model.getResource(RDF_TYPE_URI + RDF_BAG)) || resource.hasProperty(RDF.type,
                    model.getResource(RDF_TYPE_URI + RDF_SEQ))) {
                return true;
            }
        }
     ...
     * @param namespace the unrecognized namespace URI that needs a prefix
    ...
    private static Object handleExtendedPropertyValue(final Class<?> beanClass,
            final RDFNode object, Map<String, Object> visitedResources, final QName propertyQName,
            final HashSet<String> rdfTypes)
            throws URISyntaxException, IllegalArgumentException, SecurityException,
            DatatypeConfigurationException, IllegalAccessException, InstantiationException,
...

    {
        if (object.isLiteral()) {
            ...
            try {
                ...
                if ("false".equals(
                        System.getProperty(AbstractOslcRdfXmlProvider.OSLC4J_STRICT_DATATYPES))) {
                    if (logger.isLoggable(Level.WARNING)) {
                        logger.log(Level.WARNING,
                                "Property " + propertyQName.getNamespaceURI() + propertyQName
                                        .getLocalPart() + " could not be parsed as datatype " +
                                        literal
                                        .getDatatypeURI(), e);
                    }
        ...
        if ((nestedResource.getURI() == null || nestedResource.listProperties()
                .hasNext()) && (!visitedResources.containsKey(
                getVisitedResourceName(nestedResource)))) {
            final AbstractResource any = new AnyResource();
            final Map<Class<?>, Map<String, Method>> classPropertyDefinitionsToSetMethods = new
                    HashMap<>();
            fromResource(classPropertyDefinitionsToSetMethods, AnyResource.class, any,
                    nestedResource, visitedResources, rdfTypes);
        ...
        for (final Method method : methods) {
            if (method.getParameterTypes().length == 0) {
                ...
                if (((getMethodName.startsWith(
                        METHOD_NAME_START_GET)) && (getMethodName.length() >
                        METHOD_NAME_START_GET_LENGTH)) || ((getMethodName
                        .startsWith(
                                METHOD_NAME_START_IS)) && (getMethodName.length() >
                    ...
                    if (oslcPropertyDefinitionAnnotation != null) {
                        ...
                        if (getMethodName.startsWith(METHOD_NAME_START_GET)) {
                            setMethodName = METHOD_NAME_START_SET + getMethodName.substring(
                        ...
                        try {
                            ...
                            throw new OslcCoreMissingSetMethodException(beanClass, method,
                                    exception);
                        }
                    }
                }
    ...
    private static void buildResource(final Object object, final Class<?> resourceClass,
            final Model model, final Resource mainResource, final Map<String, Object> properties)
            throws DatatypeConfigurationException, IllegalAccessException, IllegalArgumentException,
            InvocationTargetException, OslcCoreApplicationException {
        ...
        for (final Method method : resourceClass.getMethods()) {
            if (method.getParameterTypes().length == 0) {
                ...
                if (((methodName.startsWith(
                        METHOD_NAME_START_GET)) && (methodName.length() >
                        METHOD_NAME_START_GET_LENGTH)) || ((methodName
                        .startsWith(
                                METHOD_NAME_START_IS)) && (methodName.length() >
                        METHOD_NAME_START_IS_LENGTH))) {
                    final OslcPropertyDefinition oslcPropertyDefinitionAnnotation =
                            InheritedMethodAnnotationHelper
                            .getAnnotation(method, OslcPropertyDefinition.class);
                        ...
                        if (value != null) {
                            ...
                            if (properties != null) {
                                ...
                                if (map != null) {
                                    ...
                                    onlyNested = !(properties instanceof
                            ...
                            buildAttributeResource(resourceClass, method,
                                    oslcPropertyDefinitionAnnotation, model, mainResource, value,
                                    nestedProperties, onlyNested);
                        }
                    }
                }
            }
        ...
        if (object instanceof IExtendedResource) {
            ...
            Map<IExtendedResource, Resource> visitedResources = new HashMap<>();
            handleExtendedProperties(resourceClass, model, mainResource, extendedResource,
                    properties, visitedResources);
        }
    }

    protected static void handleExtendedProperties(final Class<?> resourceClass, final Model model,
            final Resource mainResource, final IExtendedResource extendedResource,
            final Map<String, Object> properties, Map<IExtendedResource, Resource> visitedResources)
        ...
        for (final URI type : extendedResource.getTypes()) {
            ...
            if (properties != null && properties.get(
                    propertyName) == null && !(properties instanceof NestedWildcardProperties) &&
        ...
        for (final Map.Entry<QName, ?> extendedProperty : extendedResource.getExtendedProperties()
                .entrySet()) {
            ...
            if (properties != null) {
                @SuppressWarnings(
                        "unchecked")                final Map<String, Object> map = (Map<String,
                        Object>) properties
                        .get(propertyName);

                if (map != null) {
                    nestedProperties = map;
                } else if (properties instanceof SingletonWildcardProperties && !(properties
                        instanceof NestedWildcardProperties)) {
                    nestedProperties = OSLC4JConstants.OSL4J_PROPERTY_SINGLETON;
                } else if (properties instanceof NestedWildcardProperties) {
                    nestedProperties = ((NestedWildcardProperties) properties)
                            .commonNestedProperties();
            ...
            if (value instanceof Collection) {
                ...
                for (Object next : collection) {
                    handleExtendedValue(resourceClass, next, model, mainResource, property,
                            nestedProperties, onlyNested, visitedResources, transformer);
                }
            } else {
    ...
    private static void handleExtendedValue(final Class<?> objectClass, final Object value,
            final Model model, final Resource resource, final Property property,
            final Map<String, Object> nestedProperties, final boolean onlyNested,
            final Map<IExtendedResource, Resource> visitedResources, final Transformer transformer)
            throws DatatypeConfigurationException, IllegalAccessException, IllegalArgumentException,
            InvocationTargetException, OslcCoreApplicationException {
        if (value instanceof UnparseableLiteral) {
            ...
            if (!visitedResources.containsKey(any)) {
                ...
                if (aboutURI != null) {
                    if (OSLC4JUtils.relativeURIsAreDisabled() && !aboutURI.isAbsolute()) {
                        throw new OslcCoreRelativeURIException(AnyResource.class, "getAbout",
                                aboutURI);
                    }
                ...
                for (final URI type : any.getTypes()) {
                    ...
                    if (nestedProperties == null || nestedProperties.get(
                            propertyName) != null || nestedProperties instanceof
                            NestedWildcardProperties || nestedProperties instanceof
                            SingletonWildcardProperties) {
        ...
        } else if (value.getClass().getAnnotation(
                OslcResourceShape.class) != null || value instanceof URI) {
            //TODO:	 Until we handle XMLLiteral for incoming unknown resources, need to assume it
            // is not XMLLiteral
            handleLocalResource(objectClass, null, false, value, model, resource, property,
                    nestedProperties, onlyNested, null);
            ...
            if (dataType != null && dataType instanceof XSDDateType) {
                ...
                resource.addProperty(property,
                        model.createTypedLiteral(valueDate, XSDDatatype.XSDdate));
            } else {
    ...
    private static void buildAttributeResource(final Class<?> resourceClass, final Method method,
            final OslcPropertyDefinition propertyDefinitionAnnotation, final Model model,
            Resource resource, final Object value, final Map<String, Object> nestedProperties,
        ...
        final OslcRdfCollectionType collectionType = InheritedMethodAnnotationHelper.getAnnotation(
                method, OslcRdfCollectionType.class);
        final List<RDFNode> rdfNodeContainer;

        if (collectionType != null && OslcConstants.RDF_NAMESPACE.equals(
                collectionType.namespaceURI()) && (RDF_LIST.equals(
                collectionType.collectionType()) || RDF_ALT.equals(
                collectionType.collectionType()) || RDF_BAG.equals(
                collectionType.collectionType()) || RDF_SEQ.equals(
        ...
        if (returnType.isArray()) {
            ...
            for (int index = 0; index < length; index++) {
                ...
                handleLocalResource(resourceClass, method, xmlLiteral, object, model, resource,
                        attribute, nestedProperties, onlyNested, rdfNodeContainer);
            }
            ...
            @SuppressWarnings(
                    "unchecked")            final Collection<Object> collection =
            ...
            handleLocalResource(resourceClass, method, xmlLiteral, value, model, resource,
                    attribute, nestedProperties, onlyNested, null);
        }
    }
    ...
    private static void handleLocalResource(final Class<?> resourceClass, final Method method,
            final boolean xmlLiteral, final Object object, final Model model,
            final Resource resource, final Property attribute,
            final Map<String, Object> nestedProperties, final boolean onlyNested,
            final List<RDFNode> rdfNodeContainer)
            throws DatatypeConfigurationException, IllegalAccessException, IllegalArgumentException,
            InvocationTargetException, OslcCoreApplicationException {
        ...
        // Doubles need special handling for infinite values.
            ...
            if (OSLC4JUtils.relativeURIsAreDisabled() && !uri.isAbsolute()) {
                throw new OslcCoreRelativeURIException(resourceClass,
                        (method == null) ? "<none>" : method.getName(), uri);
            }

            // URIs represent references to other resources identified by their IDs, so they need
            ...
            // Warn that one of the properties could not be serialized because it does not have
            ...
            logger.warning(
                    "Could not serialize " + objectClassName + " because it does not have an " +
                            "OslcResourceShape annotation (class: " + subjectClassName + ", " +
                            "method: " + method
                            .getName() + ")");
        }
        ...
        if (nestedNode != null) {
            if (rdfNodeContainer != null) {
                ...
                if (reifiedResource != null && nestedProperties != OSLC4JConstants
    ...
    private static void addReifiedStatements(final Model model, final Statement statement,
            final IReifiedResource<?> reifiedResource, final Map<String, Object> nestedProperties)
        ...
        buildResource(reifiedResource, reifiedResource.getClass(), model, reifiedStatement,
                nestedProperties);
    }

    private static String getDefaultPropertyName(final Method method) {
        final String methodName = method.getName();
        final int startingIndex = methodName.startsWith(
                METHOD_NAME_START_GET) ? METHOD_NAME_START_GET_LENGTH : METHOD_NAME_START_IS_LENGTH;
        final int endingIndex = startingIndex + 1;

        // We want the name to start with a lower-case letter
        final String lowercasedFirstCharacter = methodName.substring(startingIndex, endingIndex)
    ...
    private static void recursivelyCollectNamespaceMappings(
        ...
        final OslcSchema oslcSchemaAnnotation = resourceClass.getPackage().getAnnotation(
                OslcSchema.class);

        if (oslcSchemaAnnotation != null) {
            final OslcNamespaceDefinition[] oslcNamespaceDefinitionAnnotations =
                    oslcSchemaAnnotation
                    .value();
                ...
                try {
                    ...
                    throw new RuntimeException(
                            "The custom namespace provider implementation: " +
                                    customNamespaceProvider
                                    .getClass()
                                    .getName() + ", must have a public no args construtor", e);
    ...
    private static void ensureNamespacePrefix(final String prefix, final String namespace,
        ...
        if (!namespaceMappings.containsValue(namespace)) {
            if (!namespaceMappings.containsKey(prefix)) {
                ...
                // There is already a namespace for this prefix.  We need to generate a new
                // unique prefix.
                int index = 1;
