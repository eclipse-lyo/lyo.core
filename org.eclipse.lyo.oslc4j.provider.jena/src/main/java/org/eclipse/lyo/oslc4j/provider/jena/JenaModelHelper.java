...
public final class JenaModelHelper
{
			   ...
			   OslcCoreApplicationException
	{
		...
		if (object instanceof UnparseableLiteral) {
			final UnparseableLiteral unparseable = (UnparseableLiteral) object;
			value = unparseable.getRawValue();
			reifiedResource = null;
		} else {
			reifiedResource = (object instanceof IReifiedResource) ? (IReifiedResource<?>) object : null;
			value = (reifiedResource == null) ? object : reifiedResource.getValue();
		}
