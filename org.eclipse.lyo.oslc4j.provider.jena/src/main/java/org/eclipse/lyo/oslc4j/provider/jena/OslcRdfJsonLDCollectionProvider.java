...
public class OslcRdfJsonLDCollectionProvider
	   extends AbstractOslcRdfJsonLDProvider
	   implements MessageBodyReader<Collection<Object>>,
				  MessageBodyWriter<Collection<Object>>
{
	public OslcRdfJsonLDCollectionProvider()
	{
		super();
	}
			...
			(genericType instanceof ParameterizedType))
		{
			...
			if (actualTypeArguments.length == 1)
			{
				...
				if (URI.class.equals((Class<?>) actualTypeArgument)
						&& (OslcMediaType.APPLICATION_JSON_LD_TYPE.isCompatible(mediaType))) 
				{
					return true;
				}
				else 
