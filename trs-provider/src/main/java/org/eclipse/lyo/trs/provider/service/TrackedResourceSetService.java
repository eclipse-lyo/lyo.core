...
package org.eclipse.lyo.trs.provider.service;
...
import org.eclipse.lyo.trs.provider.ChangeHistories;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

/**
 * The service class for the TRS interface. This class needs to be implemented
 * by an OSLC adapter wishing to implement a TRS interface
...
public abstract class TrackedResourceSetService {
    private final static Logger log = LoggerFactory.getLogger(TrackedResourceSetService.class);
    ...
    @Produces(
            {OslcMediaType.TEXT_TURTLE, OslcMediaType.APPLICATION_RDF_XML, OslcMediaType
                    .APPLICATION_XML, OslcMediaType.APPLICATION_JSON})
    public Response getBasePage(@PathParam("page") String pagenum) {
        log.info(
                "received request for base page at url:" + httpServletRequest.getRequestURI() +
                        "" + " . Processing request");
        Base base;
        try {
        ...
        log.info(
                "finished processing request for base page at url:" + httpServletRequest
                        .getRequestURI() + " .");
        log.info(
                "the returned base at url" + httpServletRequest.getRequestURI() + " contains :" +
                        base
                        .getMembers()
                        .size() + " members.");
    ...
    public ChangeLog getChangeLogPage(@PathParam("page") String pagenum) {
        log.info(
                "received request for changeLog page at url:" + httpServletRequest.getRequestURI
                        () + " . Processing request");
        ...
        log.info(
                "finished processing request for changeLog page at url:" + httpServletRequest
                        .getRequestURI() + " .");
        log.info(
                "the returned change with url" + httpServletRequest.getRequestURI() + " log " +
                        "contains :" + changeLog
                        .getChange()
                        .size() + " changes.");
        return changeLog;
    ...
    /**
     * The instance of the change histories class used by a trs service class
     * implementing this class. The instance returned is expected to be a
     * singleton of a class implementing the ChangeHistories class
    ...
    public TrackedResourceSet getTrackedResourceSet() throws URISyntaxException {
        log.info(
                "received request for trs at url:" + httpServletRequest.getRequestURI() + " . " +
                        "Processing request");
        TrackedResourceSet result = new TrackedResourceSet();
        result.setAbout(URI.create(getServiceBase() + "/trs"));//$NON-NLS-1$
        result.setBase(
                URI.create(getServiceBase() + "/trs/" + TRSConstants.TRS_TERM_BASE));//$NON-NLS-1$
        ...
        log.info(
                "returning response for request for trs with url:" + httpServletRequest
                        .getRequestURI() + " .");
        log.info(
                "the returned change log at url" + httpServletRequest.getRequestURI() + " " +
    ...
    /**
     * manage http calls for the first page of the base. The call is redirected
     * to the handler of http calls for a specific base page as a call for the
     * page 1 of the base
    ...
    public Page getBase() {
        ...
        try {
            throw new WebApplicationException(
                    Response.temporaryRedirect(new URI(redirectLocation)).build());
        } catch (URISyntaxException e) {
            throw new IllegalStateException(e);
        }
    }
    ...
    /**
     * manage the calls for the change log and redirects to the handler of a
     * specific page of the change log with the call to the first page
    ...
    private String linkHeaderValue(Base base) {
        ...
        String headerValue = urize(pageOf.toString()) + "; rel=\"first\"," + newline + urize(
                nextPage.getNextPage()
                        .toString()) + "; rel=\"next\"," + newline + "<http://www" + "" + "" + "" +
