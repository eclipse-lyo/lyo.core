...
public class OslcGlobalNamespaceProviderTest {
	...
	/**
	 * Tests if the map can be setted to another new map or to null. 
	 */
	@Test
	public void testSetNullMap() {
		...
		Assert.assertNotNull(
				"Global Namespace Map can not be set to null.",
				globalNamespaceProvider.getPrefixDefinitionMap()
		);
		Assert.assertTrue(
				"Map should be cleared and never setted to null.",
		...
		Assert.assertFalse(
				"Global Namespace Map could not be setted.", 
				globalNamespaceProvider.getPrefixDefinitionMap().isEmpty()
		);
	}
